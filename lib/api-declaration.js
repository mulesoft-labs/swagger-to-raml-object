...
function convertParameterToJson (param, declaration) {
  ...
  [
    ...
    if (key === 'items') {
      return schema[key] = param[key].map(function (item) {
        return convertParameterToJson(item, declaration);
      });
    }
...
function convertParameterToJsonSchema (param, declaration) {
  return JSON.stringify(convertParameterToJson(param, declaration), null, 2);
}
